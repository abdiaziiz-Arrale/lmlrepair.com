...
import { getLatestPosts, getPublishedPosts } from '@/lib/db/postCrud';
...
const PostPage = () => {
  ...
  const loadPosts = async () => {
    try {
      ...
      const latestPosts:any  = await getLatestPosts();
      setPosts(Posts);
  ...
  const fetchMorePosts = async () => {
    try {
      ...
      if (posts && posts.length > 0) {
        const updatedPosts:any = [...posts, ...posts];
        setPosts(updatedPosts);
        setVisiblePostsCount(updatedPosts.length); 
      } else {
      ...
      <main className=" mx-12 py-12 px-4 md:px-1 grid grid-cols-1 gap-8 lg:grid-cols-3">
        <section className="lg:col-span-2 space-y-8">
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {posts.slice(0, visiblePostsCount).map((post: Post) => (
                  <Link href={`/Blogpost/${post.id}`} key={post.id} passHref>
          ...
          <div className="rounded-lg bg-muted p-4">
            ...
            <div className="mt-4 space-y-4">
              {Latestposts.map((post: Post) => (
                <Link href={`/Blogpost/${post.id}`} key={post.id} className="group grid grid-cols-[48px_1fr] items-center gap-4" prefetch={false}>
                  <img
                    src={logo.src}
